...
def version_instance_info(entity_instance, instances, *args, **kwargs):
    ...
    for instance in instances:
        ret_string +=  '<p>"' + instance.name + '" (<a href="' + reverse('api_dataset', args=(base64_EntityInstance_URIInstance,"html")) + '">browse the data</a> or'
        ...
        ret_string += 'Version ' + ('' if entity_instance.version_released else '(<font color="red">not released</font>) ') + str(entity_instance.version_major) + '.' + str(entity_instance.version_minor) + '.' + str(entity_instance.version_patch) + ' - ' + str(entity_instance.version_date)
